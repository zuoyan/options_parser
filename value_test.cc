...
#define get_error_check(V)                                 \
  ({                                                       \
    ASSERT(options_parser::is_error(V), "expect a error"); \
    get_error(V);                                          \
  })

#define get_value_check(V)                                             \
  ({                                                                   \
    ASSERT(!options_parser::is_error(V), "got a error", get_error(V)); \
    get_value(V);                                                      \
  })
...
#define CHECK_VALUE(VALUE, SITUATION, ERROR, V, INDEX, OFF)             \
  ({                                                                    \
    ...
    if (error_s.size()) {                                               \
      ASSERT(is_error(v_s.first), "expect a error", error_s);           \
      CHECK_EQ(get_error(v_s.first), error_s, "error not expected");    \
    } else {                                                            \
      ASSERT(is_value(v_s.first), "get a error", get_error(v_s.first)); \
...
TEST(BasicString) {
  ...
  CHECK_EQ(get_value_check(v_s.first), "a string value");
  v_s = value(to_situation());
  CHECK_EQ(v_s.second.position.index, 0);
  CHECK_EQ(v_s.second.position.off, 0);
  CHECK_EQ(get_error_check(v_s.first), "no arguments rest");
}
...
TEST(BasicInt) {
  ...
  CHECK_EQ(v_s.second.position.index, 1);
  CHECK_EQ(v_s.second.position.off, 0);
  CHECK_EQ(get_value_check(v_s.first), 13);
  v_s = value(to_situation());
  CHECK_EQ(v_s.second.position.index, 0);
  CHECK_EQ(v_s.second.position.off, 0);
  CHECK_EQ(get_error_check(v_s.first), "no arguments rest");
  ...
  CHECK_EQ(v_s.second.position.index, 0);
  CHECK_EQ(v_s.second.position.off, 0);
  CHECK_EQ(get_error_check(v_s.first), "from_str<i>(\"13a\") rest \"a\"");
}
...
TEST(BasicBool) {
  ...
  CHECK_EQ(v_s.second.position.off, 0);
  CHECK_EQ(test::to_container(get_value_check(v_s.first)),
...
TEST(ManyInt) {
  ...
  CHECK_EQ(v_s.second.position.index, 0);
  CHECK_EQ(v_s.second.position.off, 0);
  CHECK_EQ(test::to_container(get_value_check(v_s.first)), test::Container<int>());
...
TEST(Gather) {
  ...
  static_assert(std::tuple_size<decltype(get_value(v_s.first))>::value == 3,
                "fix me first");
  CHECK_EQ(std::get<0>(get_value_check(v_s.first)), 13);
  CHECK_EQ(std::get<1>(get_value_check(v_s.first)), "a string");
  CHECK_EQ(std::get<2>(get_value_check(v_s.first)), 3.14);

  v_s = value(to_situation("13", "a string", "3.14a"));
  CHECK_EQ(v_s.second.position.index, 0);
  CHECK_EQ(v_s.second.position.off, 0);
  CHECK_EQ(get_error_check(v_s.first), "from_str<d>(\"3.14a\") rest \"a\"");
}
...
TEST(Check) {
  ...
  CHECK_EQ(v_s.second.position.index, 0);
  CHECK_EQ(v_s.second.position.off, 0);
  CHECK_EQ(get_error_check(v_s.first), "should starts with -");
